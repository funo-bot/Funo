...
module.exports.run = async (bot, message, args) => {
  ...
  if (isNaN(args[0]) || args[0] >= 100 || args[0] <= 0) {
    return message.channel
      .send(
        new Discord.RichEmbed()
          .setDescription(
            "Make sure to provide a valid number between **1** and **100** as first argument."
          )
          .setColor("RED")
      )
      .then(msg => msg.delete(7000));
  ...
  message.channel
    .send(`Deleting ${fetchedSize} message${fetchedSize !== 1 ? "s" : ""}...`)
    .then(msg => msg.delete(7000));
  await message.channel
    .bulkDelete(
      mentioned
        ? fetched.filter(msg => msg.author.id === mentioned.id)
        : fetched
    )
